...
当前支持：deepseek、siliconflow、ollama、openrouter、cstcloud（科技云）。
"""
...
class LLMClient:
    ...
    def rerank(
        ...
        """重排序接口（默认不支持，只有 SiliconFlow 提供）。"""
        raise NotImplementedError("rerank 仅支持 SiliconflowClient，请使用 SiliconflowClient 调用。")
...
class OpenRouterClient(LLMClient):
    """OpenRouter 网关，OpenAI Chat Completions 兼容接口。"""
    def __init__(self, api_key: str, model: str, base_url: str = None):
        legacy_base = base_url or os.getenv('OPENROUTER_API_BASE', DEFAULT_OPENROUTER_BASE_URL)
        primary_base = (
            os.getenv("LLM_PRIMARY_BASE_URL")
            or os.getenv("OPENROUTER_PRIMARY_BASE_URL")
...
class ClientFactory:
    ...
    def from_env():
        ...
        if provider in ('openrouter',):
            return OpenRouterClient(api_key=api_key or os.getenv('OPENROUTER_API_KEY', ''), model=model, base_url=base_url or os.getenv('OPENROUTER_API_BASE', DEFAULT_OPENROUTER_BASE_URL))
        ...
        raise ValueError(f"不支持的提供商: {provider}，请使用 'deepseek'、'siliconflow'、'openrouter'、'cstcloud' 或 'ollama'")
